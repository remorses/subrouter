...
- `opencode/` — npm package `@subrouter/opencode`. Plugin `config` hook injects a `subrouter` provider whose `npm` field is a `file://` URL to the bundled `provider.js`; every preset becomes a model (`subrouter/<preset>`). Visible provider name is `subrouter.org`. Model labels stay as preset names, while context limits and `experimental.chat.system.transform` follow the first live routed candidate. Only plugin initializers may be exported from `src/index.ts` (opencode calls every export as a plugin).
